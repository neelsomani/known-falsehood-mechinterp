...
def load_w(npz_path: Path, dtype: torch.dtype) -> torch.Tensor:
    z = np.load(npz_path)
    w = torch.from_numpy(z["w"]).to(dtype=dtype)
    w = w / (w.norm() + 1e-8)
    return w
...
def statement_final_token_index(tokenizer, prompt: str, statement: str) -> int:
    ...
    offsets = normalize_offsets(tokenized["offset_mapping"])
...
def run_truth_task(
    ...
    print("Truth/False/Unknown task (ablated):")
...
def run_consequence_task(
    ...
    for row in rows:
        ...
        tok_idx = statement_final_token_index(tokenizer, full_prompt, row["statement"])
        response = generate_with_ablation(
            model,
            tokenizer,
            full_prompt,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run truth and consequence behavioral checks with direction ablation."
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument("--layer", default="15", help="Layer index or 'last'.")
    parser.add_argument("--alpha", type=float, default=1.0)
    parser.add_argument("--w", default="dataset/stance_direction.npz")
...
def main() -> None:
    ...
    model.eval()
    w = load_w(Path(args.w), dtype=torch_dtype)

    layer_arg = args.layer
    if str(layer_arg).lower() == "last":
